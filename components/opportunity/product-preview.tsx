...
export function ProductPreview() {
  ...
  return (
    <section className="faust-surface p-6"><h2 className="text-xl font-semibold">Imported product</h2><p className="mt-1 text-sm text-muted-foreground">The factual product record captured from Superbuy.</p>
      <div className="mt-6 flex h-72 items-center justify-center overflow-hidden rounded-3xl border border-slate-700/45 bg-black/35">{image ? <img src={image} alt={product.name} className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : <div className="text-center text-muted-foreground"><ImageIcon className="mx-auto h-10 w-10 text-[#c8d2e6]" /><p className="mt-3 text-sm">No product image was available</p></div>}</div>
      <div className="mt-4 rounded-2xl border border-slate-700/35 bg-black/25 p-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button type="button" onClick={addImage} className="mt-auto rounded-full bg-[#56627f] px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/30 transition hover:bg-[#66708d] disabled:cursor-not-allowed disabled:opacity-60" disabled={!imageUrl.trim()}>
            Add image
          </button>
        </div>
        <label className="mt-3 block cursor-pointer rounded-2xl border border-dashed border-slate-700/60 bg-zinc-950/40 p-4 text-center text-sm text-muted-foreground transition hover:border-slate-400/50 hover:text-[#f6f8ff]">
        ...
        {product.media.images.length ? (
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
            {product.media.images.slice(0, 15).map((src, index) => (
              <div key={src} className="overflow-hidden rounded-2xl border border-slate-700/45 bg-zinc-950/60">
                ...
                <div className="grid grid-cols-2 gap-1 p-2 text-[11px]">
                  <button type="button" onClick={() => makePrimary(src)} disabled={index === 0} className="rounded-full border border-slate-700/60 px-2 py-1 text-[#f6f8ff] transition hover:border-slate-400/50 disabled:cursor-default disabled:opacity-50">
                    {index === 0 ? "Primary" : "Make first"}
                  </button>
                  <button type="button" onClick={() => removeImage(src)} className="rounded-full border border-slate-700/60 px-2 py-1 text-[#f6f8ff] transition hover:border-slate-400/50">
                    Remove
                  </button>
                </div>
              </div>
