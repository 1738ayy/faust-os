    ...
    <section className="faust-surface p-6"><h2 className="text-xl font-semibold">Imported product</h2><p className="mt-1 text-sm text-muted-foreground">The factual product record captured from Superbuy.</p>
      ...
      <div className="mt-4 rounded-2xl border border-red-950/35 bg-black/25 p-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button type="button" onClick={addImage} className="mt-auto rounded-full bg-red-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-red-950/30 transition hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-60" disabled={!imageUrl.trim()}>
            Add image
          </button>
        </div>
        <label className="mt-3 block cursor-pointer rounded-2xl border border-dashed border-red-950/60 bg-zinc-950/40 p-4 text-center text-sm text-muted-foreground transition hover:border-red-500/50 hover:text-red-100">
            ...
            {product.media.images.slice(0, 15).map((src, index) => (
              <div key={src} className="overflow-hidden rounded-2xl border border-red-950/45 bg-zinc-950/60">
                ...
                <div className="grid grid-cols-2 gap-1 p-2 text-[11px]">
                  ...
                  <button type="button" onClick={() => removeImage(src)} className="rounded-full border border-red-950/60 px-2 py-1 text-red-100 transition hover:border-red-500/50">
                    Remove
                  </button>
                </div>
              </div>
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-2"><a href={product.sourcing.superbuyUrl} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-full border border-red-950/60 bg-zinc-950/50 px-3 py-2 text-sm font-medium transition hover:border-red-500/50 hover:text-white"><ExternalLink className="h-4 w-4" />Superbuy</a>{product.sourcing.original1688Url && <a href={product.sourcing.original1688Url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-full border border-red-950/60 bg-zinc-950/50 px-3 py-2 text-sm font-medium transition hover:border-red-500/50 hover:text-white"><ExternalLink className="h-4 w-4" />Original listing</a>}</div>
    </section>
  );
}
