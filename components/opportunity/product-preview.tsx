"use client";
/* eslint-disable @next/next/no-img-element -- The crop editor needs direct access to the rendered image bounds for precise canvas output. */
...
type CropRecord = { original: string; transform: CropTransform; croppedAt: string };
type CropState = { index: number; source: string; original: string; box: CropBox; ratio: CropRatio; zoom: number; drag?: { mode: DragMode; pointerId: number; startX: number; startY: number; startBox: CropBox } };
...
function cropMetadataKey(importQueueItemId?: string) {
  return importQueueItemId ? `faust.importQueueCropTransforms.${importQueueItemId}` : undefined;
}

function readCropRecords(importQueueItemId?: string): Record<string, CropRecord> {
  const key = cropMetadataKey(importQueueItemId);
  if (!key || typeof window === "undefined") return {};
  try {
    const parsed = JSON.parse(window.localStorage.getItem(key) || "{}") as Record<string, CropRecord>;
...
function saveCropRecord(importQueueItemId: string | undefined, editedImage: string, record: CropRecord) {
  const key = cropMetadataKey(importQueueItemId);
  if (!key || typeof window === "undefined") return;
  try {
    const records = readCropRecords(importQueueItemId);
    window.localStorage.setItem(key, JSON.stringify({ ...records, [editedImage]: record, [record.original]: record }));
  } catch {
    // Crop metadata is a recoverability layer; the cropped image itself remains saved in the draft.
  }
}
...
function resizeLocked(box: CropBox, mode: DragMode, dxPct: number, dyPct: number, stage: DOMRect, ratio: CropRatio) {
  ...
  const start = {
    x: (box.x / 100) * stage.width,
    y: (box.y / 100) * stage.height,
    width: (box.width / 100) * stage.width,
    height: (box.height / 100) * stage.height,
  };
...
export function ProductPreview() {
  ...
  const importQueueItemId = opportunity.importQueueItemId;
  ...
  function openCrop(index: number) {
    ...
    setCropState({
      ...
      source,
      original,
      box: record?.transform.box || defaultCropBox,
      ratio: record?.transform.ratio || "free",
      zoom: record?.transform.zoom || 1,
    });
  }
  ...
  function dragCrop(event: ReactPointerEvent<HTMLElement>) {
    ...
    let box = cropState.drag.startBox;
    if (cropState.drag.mode === "move") box = clampBox({ ...box, x: box.x + dx, y: box.y + dy }, stage);
    else box = cropState.ratio === "free" ? resizeFree(box, cropState.drag.mode, dx, dy, stage) : resizeLocked(box, cropState.drag.mode, dx, dy, stage, cropState.ratio);
  ...
  function setCropRatio(ratio: CropRatio) {
    ...
    setCropState((current) => {
      ...
      return { ...current, ratio, box: stage ? cropBoxForRatio(current.box, ratio, stage) : current.box };
    });
  }
  ...
  async function saveCrop() {
    ...
    try {
      ...
      const boxRect = {
        left: stage.left + (cropState.box.x / 100) * stage.width,
        top: stage.top + (cropState.box.y / 100) * stage.height,
        width: (cropState.box.width / 100) * stage.width,
        height: (cropState.box.height / 100) * stage.height,
      };
      const image = await loadBrowserImage(proxiedImage(cropState.original));
      ...
      saveCropRecord(importQueueItemId, editedImage, { original: cropState.original, transform: { box: cropState.box, ratio: cropState.ratio, zoom: cropState.zoom }, croppedAt: new Date().toISOString() });
      setImages(images.map((src, index) => index === cropState.index ? editedImage : src));
      setCropState(null);
    } catch {
  ...
  function handleCropKeys(event: React.KeyboardEvent<HTMLElement>) {
    ...
    const next = resize
      ? clampBox({ ...cropState.box, width: cropState.box.width + deltaX, height: cropState.box.height + deltaY }, stage)
      : clampBox({ ...cropState.box, x: cropState.box.x + deltaX, y: cropState.box.y + deltaY }, stage);
    updateCropBox(next);
  }
      ...
      {cropState ? (
        <div role="dialog" aria-modal="true" aria-label="Crop photo" className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-5" onKeyDown={handleCropKeys}>
          <div className="faust-surface w-full max-w-4xl p-5 shadow-2xl shadow-slate-950/80">
            ...
            <div
              ref={stageRef}
              className="relative mt-5 flex h-[min(68vh,680px)] touch-none select-none items-center justify-center overflow-hidden rounded-3xl border border-slate-700/45 bg-black/45 outline-none"
              tabIndex={0}
              role="application"
              aria-label="Photo crop area. Drag the crop box or handles. Arrow keys move the crop. Alt plus arrow resizes. Enter saves. Escape cancels."
              ...
              onWheel={(event) => {
                ...
                event.preventDefault();
                setCropZoom(cropState.zoom + (event.deltaY < 0 ? 0.05 : -0.05));
              }}
            >
              <img ref={imageRef} src={proxiedImage(cropState.original)} alt="" className="max-h-full max-w-full object-contain transition-transform" style={{ transform: `scale(${cropState.zoom})` }} draggable={false} />
              ...
              <div
                role="region"
                aria-label="Selected crop area"
                className="absolute cursor-move border border-[#c8d2e6] shadow-[0_0_0_1px_rgba(200,210,230,0.25),0_0_30px_rgba(102,112,141,0.35)] focus:outline-none"
                style={{ left: `${cropState.box.x}%`, top: `${cropState.box.y}%`, width: `${cropState.box.width}%`, height: `${cropState.box.height}%` }}
                tabIndex={0}
              ...
              >
                ...
                {(["nw", "ne", "sw", "se"] as const).map((corner) => (
                  <button
                    ...
                    className={`absolute h-5 w-5 rounded-full border border-[#f6f8ff] bg-[#66708d] shadow-lg shadow-black/40 ${corner.includes("n") ? "-top-2.5" : "-bottom-2.5"} ${corner.includes("w") ? "-left-2.5" : "-right-2.5"}`}
                ...
                {(["n", "s", "e", "w"] as const).map((edge) => (
                  <button
                    key={edge}
                    type="button"
                    aria-label={`Resize crop from ${edge.toUpperCase()} edge`}
                    className={`absolute rounded-full border border-[#c8d2e6]/80 bg-[#66708d]/90 ${edge === "n" || edge === "s" ? "left-1/2 h-3 w-12 -translate-x-1/2" : "top-1/2 h-12 w-3 -translate-y-1/2"} ${edge === "n" ? "-top-1.5" : ""} ${edge === "s" ? "-bottom-1.5" : ""} ${edge === "e" ? "-right-1.5" : ""} ${edge === "w" ? "-left-1.5" : ""}`}
                    style={{ cursor: edge === "n" || edge === "s" ? `${edge}-resize` : `${edge}-resize` }}
