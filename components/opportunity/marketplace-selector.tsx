...
export function MarketplaceSelector() {
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-6">
      <div className="flex items-center gap-3"><Store className="h-6 w-6 text-violet-400" /><div><h2 className="text-xl font-semibold">Marketplace</h2><p className="text-sm text-muted-foreground">Choose the first place you plan to list this product.</p></div></div>
      <select value={marketplace.id} onChange={(event) => updateMarketplace(event.target.value as typeof marketplace.id)} className="mt-6 h-12 w-full rounded-xl border border-border bg-background px-4">
