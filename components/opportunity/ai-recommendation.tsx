...
export function AiRecommendation() {
  ...
  return <section className="rounded-xl border border-border bg-card p-6"><div className="flex items-center gap-3"><BrainCircuit className="h-7 w-7 text-violet-400" /><div><h2 className="text-xl font-semibold">Decision Summary</h2><p className="text-sm text-muted-foreground">Rule-based guidance until an AI provider is connected.</p></div></div><div className="mt-8 flex items-center gap-3"><Icon className={`h-6 w-6 ${healthy ? "text-emerald-400" : "text-yellow-400"}`} /><h3 className={`text-xl font-bold ${healthy ? "text-emerald-400" : "text-yellow-400"}`}>{healthy ? "Ready for a test order" : "Needs review"}</h3></div><div className="mt-6 space-y-3">{advice.map((item) => <div key={item} className="rounded-lg bg-muted/40 p-3">{item}</div>)}</div><div className="mt-8 rounded-xl bg-violet-500/10 p-4"><p className="text-sm text-muted-foreground">Suggested sale price</p><p className="mt-2 text-2xl font-bold">${suggestedPrice.toFixed(2)}</p></div></section>;
}
