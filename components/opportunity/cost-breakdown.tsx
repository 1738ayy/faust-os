...
const editableCosts: CostKey[] = ["product", "domesticShipping", "internationalShipping", "packaging", "taxes", "storage", "warehouse", "returns", "miscellaneous"];
...
export function CostBreakdown() {
  ...
  return (
    <section className="rounded-3xl border border-slate-700/45 bg-zinc-950/55 p-6 shadow-lg shadow-black/20 backdrop-blur">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Enter sourcing and fulfillment costs. Marketplace selling costs are loaded from the selected channel profile.</p>
      <div className="mt-7 space-y-5">
        ...
        <div className="rounded-xl bg-muted/40 p-4 text-sm">
          <div className="flex justify-between"><span>Marketplace selling costs</span><span>${analysis.marketplaceFees.toFixed(2)}</span></div>
          ...
          <div className="mt-2 flex justify-between"><span>Promotion / Boost</span><span>${analysis.promotionFees.toFixed(2)}</span></div>
        </div>
      </div>
