...
export function FinancialResults() {
  ...
  return (
    <section className="faust-surface p-6">
      ...
      <div className="faust-card mt-6 space-y-2 p-4 text-sm"><div className="flex justify-between"><span>Revenue</span><span>${analysis.revenue.toFixed(2)}</span></div><div className="flex justify-between"><span>Gross profit</span><span>${analysis.grossProfit.toFixed(2)}</span></div><div className="flex justify-between"><span>Target margin</span><span>{analysis.targetMargin.toFixed(0)}%</span></div><div className="flex justify-between"><span>Target price</span><span>${analysis.recommendedPrice.toFixed(2)}</span></div><div className="flex justify-between font-medium"><span>Capital required</span><span>${analysis.capitalRequired.toFixed(2)}</span></div></div>
    </section>
  );
}
