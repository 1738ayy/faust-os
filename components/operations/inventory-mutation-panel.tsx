...
const actionButton = "rounded-full border border-slate-700/60 bg-zinc-950/50 px-3 py-2 text-xs font-medium transition hover:border-slate-400/50 hover:text-white";
...
export function InventoryMutationPanel({ balances, locations }: { balances: StockBalance[]; locations: Location[] }) {
  ...
  return <section id="inventory-actions" className="rounded-3xl border border-slate-700/45 bg-zinc-950/55 p-5 shadow-lg shadow-black/20 backdrop-blur scroll-mt-24" aria-label="Inventory actions">
    ...
    <form className="mt-5 grid gap-3 md:grid-cols-3" onSubmit={submit}>
      ...
      <button disabled={submitting || (requiresQuantity && !balances.length)} className="rounded-full bg-[#56627f] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-slate-950/30 transition hover:bg-[#66708d] disabled:opacity-50">{submitting ? "Saving…" : `Confirm ${selected.label}`}</button>
    </form>
    {message && <p role="status" className="mt-4 rounded-2xl border border-slate-700/35 bg-black/35 p-3 text-sm text-muted-foreground">{message}</p>}
  </section>;
}
