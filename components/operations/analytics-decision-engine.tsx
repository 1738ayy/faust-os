...
const Header = ({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) => (
  <header>
    <p className="text-sm font-medium text-red-300">{eyebrow}</p>
...
const Panel = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <section className="faust-surface overflow-hidden">
    <h2 className="border-b border-red-950/45 px-5 py-4 font-semibold">{title}</h2>
    {children}
  </section>
);

const Metric = ({ label, value, href }: { label: string; value: string; href?: string }) => (
  <article className="faust-card p-5 transition hover:border-red-500/35">
    <p className="text-sm text-muted-foreground">{label}</p>
    <p className="mt-3 font-heading text-2xl font-semibold tabular-nums">{value}</p>
    {href && <Link className="mt-3 inline-block text-xs font-medium text-red-300 hover:text-red-200" href={href}>Open source records →</Link>}
  </article>
);
...
const DecisionCallout = ({ analytics }: { analytics: AnalyticsModel }) => {
  ...
  return <section className="rounded-3xl border border-red-950/45 bg-zinc-950/55 p-5 shadow-lg shadow-black/20 backdrop-blur"><p className="text-xs font-medium uppercase tracking-[0.16em] text-red-300">Faust recommendation</p><h2 className="mt-2 text-xl font-semibold">{title}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-muted-foreground">{detail}</p></section>;
};

const Row = ({ label, value, href }: { label: string; value: React.ReactNode; href?: string }) => (
  <div className="flex justify-between gap-4 border-b border-red-950/35 px-5 py-3 text-sm last:border-0">
    <span className="text-muted-foreground">{href ? <Link className="text-red-300 hover:text-red-200" href={href}>{label}</Link> : label}</span>
  ...
  return (
    <Panel title="Reporting controls">
      <form className="grid gap-3 p-5 md:grid-cols-6" action="/analytics">
        ...
        <button className="self-end rounded-full bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-lg shadow-red-950/30 transition hover:bg-red-500">Apply filters</button>
        <Link className="self-end rounded-full border border-red-950/60 bg-zinc-950/50 px-3 py-2 text-center text-sm transition hover:border-red-500/50" href={`/api/exports/analytics?${new URLSearchParams(Object.entries(filters).filter(([, value]) => Boolean(value)) as [string, string][]).toString()}`}>Export CSV</Link>
      </form>
    </Panel>
  );
}
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left text-sm">
          ...
          <tbody>{analytics.products.map((product) => (
            <tr className="border-t border-red-950/35" key={product.variantId}>
              <td className="px-5 py-3"><Link className="text-red-300 hover:text-red-200" href={product.sourceHref}>{product.sku}</Link><p className="text-xs text-muted-foreground">{product.title}</p></td>
              ...
              <td className="px-5 py-3"><Link className="text-red-300 hover:text-red-200" href={product.lotHref}>Lot profitability</Link><p className="text-xs text-muted-foreground">{product.lotProfitability.map((lot) => `${lot.capitalUtilization}% utilized`).join(separator) || "No lots"}</p></td>
