...
const primaryButton = "rounded-full bg-[#56627f] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-slate-950/30 transition hover:bg-[#66708d] disabled:opacity-60";
  ...
  return (
    <section className="faust-surface p-5" aria-label="Order import workspace">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-[#edf3ff]">Order import</p>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        <div className="faust-card overflow-hidden">
          <div className="border-b border-slate-700/35 p-4 text-sm font-semibold">Import batch history</div>
          {batches.map((batch) => (
            <button type="button" key={batch.id} onClick={() => setActiveBatchId(batch.id)} className={`block w-full border-b border-slate-700/30 p-4 text-left text-xs transition hover:bg-[#66708d]/5 ${activeBatch?.id === batch.id ? "bg-[#66708d]/10" : ""}`}>
              <b>{batch.filename}</b>
              <span className="ml-2 rounded-full border border-slate-700/45 bg-black/35 px-2 py-0.5 capitalize text-[#f6f8ff]">{batch.status.replaceAll("_", " ")}</span>
        ...
        <div className="faust-card overflow-hidden">
          <div className="flex flex-wrap items-center gap-2 border-b border-slate-700/35 p-4">
          ...
          {activeBatch ? (
            <div className="max-h-[460px] overflow-auto divide-y divide-slate-700/30">
                ...
                return (
                  <div className="grid gap-3 p-4 text-xs md:grid-cols-[1fr_auto]" key={row.id}>
                    <div>
                      ...
                      <span className="ml-2 rounded-full border border-slate-700/45 bg-black/35 px-2 py-0.5 capitalize text-[#f6f8ff]">{row.status.replaceAll("_", " ")}</span>
                      ...
                      {row.error && <p className="mt-1 text-[#c8d2e6]">{row.error}</p>}
