...
export function ListingsActionsPanel({ data }: { data: OperatingData }) {
  ...
  return <section aria-label="Listings workflows" className="border border-border bg-card">
    <h2 className="border-b border-border px-5 py-4 font-semibold">Listings Workflows</h2>
    <div className="grid gap-5 p-5 lg:grid-cols-2">
      <form className="border border-border p-4" onSubmit={(event) => { event.preventDefault(); const fields = new FormData(event.currentTarget); void run("create-five-drafts", { variantId: String(fields.get("variantId")), physicalSku: String(fields.get("physicalSku")), basePrice: Number(fields.get("basePrice")), imageUrls: [String(fields.get("imageUrl"))].filter(Boolean) }); }}>
