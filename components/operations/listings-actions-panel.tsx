...
export function ListingsActionsPanel({ data }: { data: OperatingData }) {
  ...
  const depopDraft = data.channelListingDrafts?.find((entry) => entry.marketplace === "Depop") || draft;
  ...
  return <section aria-label="Listings workflows" className="faust-surface overflow-hidden">
    <h2 className="border-b border-slate-700/45 px-5 py-4 font-semibold">Listings Workflows</h2>
    <div className="grid gap-5 p-5 lg:grid-cols-2">
      ...
      <div className="faust-card p-4">
        ...
        <p className="mt-2 text-xs text-muted-foreground">Depop uses the production connector boundary. Other unsupported channels enter guided extension/manual mode until credentials are connected.</p>
        <div className="mt-3 flex flex-wrap gap-2"><button className={button} disabled={busy === "connect-depop"} onClick={() => run("connect-depop", { displayName: "Depop production", tokenRef: "env:DEPOP_API_KEY" })}>Connect Depop</button><button className={button} disabled={!depopDraft || busy === "publish-draft"} onClick={() => depopDraft && run("publish-draft", { draftId: depopDraft.id })}>Publish Depop draft</button><button className={button} disabled={!manualDraft} onClick={() => manualDraft && run("confirm-external", { draftId: manualDraft.id, externalListingId: `${manualDraft.marketplace.toUpperCase()}-MANUAL-1`, externalUrl: `https://example.test/${manualDraft.marketplace.toLowerCase()}/manual-1` })}>Confirm external ID/URL</button></div>
      </div>
      <div className="faust-card p-4">
