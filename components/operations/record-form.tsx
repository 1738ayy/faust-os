...
export function RecordForm({ endpoint, submitLabel, fields }: { endpoint: string; submitLabel: string; fields: Field[] }) {
  ...
  return <form onSubmit={submit} className="rounded-3xl border border-slate-700/45 bg-zinc-950/55 p-5 shadow-lg shadow-black/20 backdrop-blur"><div className="grid gap-3 sm:grid-cols-2">{fields.map((field) => <label key={field.name} className="text-sm font-medium">{field.label}<input required={field.required} type={field.type || "text"} value={values[field.name] || ""} onChange={(event) => setValues((current) => ({ ...current, [field.name]: event.target.value }))} className="faust-field faust-focus mt-2 w-full px-3 py-2 font-normal" /></label>)}</div>{error && <p className="mt-3 text-sm text-red-400">{error}</p>}<button disabled={saving} className="mt-5 rounded-full bg-[#56627f] px-4 py-2 text-sm font-medium text-white shadow-lg shadow-slate-950/30 transition hover:bg-[#66708d] disabled:opacity-60">{saving ? "Saving…" : submitLabel}</button></form>;
}
