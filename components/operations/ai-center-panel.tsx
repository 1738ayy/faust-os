...
export function AiCenterPanel({ data }: { data: OperatingData }) {
  ...
  return <section aria-label="AI Center actions" className="border border-border bg-card p-5">
    <div className="flex flex-wrap items-end gap-3">
      ...
      <button disabled={busy} className="border border-emerald-500 px-4 py-2 text-sm text-emerald-300 disabled:opacity-50" onClick={() => run({ action: "ask-question", question, saveQuestion: true, provider: "deterministic" }, "Grounded answer saved.")}>Ask question</button>
      <button disabled={busy} className="border border-border px-4 py-2 text-sm" onClick={() => run({ action: "daily-brief", provider: "deterministic" }, "Daily brief generated.")}>Generate daily brief</button>
