...
export function AiCenterPanel({ data, provider }: { data: OperatingData; provider: AiProviderKey }) {
  ...
  return (
    <section aria-label="AI Center actions" className="overflow-hidden rounded-3xl border border-slate-700/45 bg-zinc-950/60 shadow-xl shadow-black/20 backdrop-blur">
      <div className="grid gap-5 border-b border-slate-700/45 p-5 lg:grid-cols-[1fr_280px]">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#c8d2e6]">
            <Bot size={15} />
            Ask Faust
          </div>
        ...
        <div className="rounded-2xl border border-slate-400/25 bg-[#66708d]/10 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#c8d2e6]">Provider</p>
      ...
      <div className="p-5">
        <label className="block text-sm font-medium">
          Question
          <textarea
            className="mt-2 min-h-24 w-full resize-none rounded-2xl border border-slate-700/45 bg-black/35 px-4 py-3 text-sm font-normal leading-6 transition focus:border-slate-400/60"
            value={question}
            onChange={(event) => setQuestion(event.target.value)}
          />
        </label>
        <div className="mt-4 flex flex-wrap gap-2">
          <button disabled={busy || !question.trim()} className="inline-flex items-center gap-2 rounded-full bg-[#56627f] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-slate-950/30 transition hover:bg-[#66708d] disabled:opacity-50" onClick={() => run({ action: "ask-question", question, saveQuestion: true, provider }, "Grounded answer saved and shown below.")}>
            <Send size={15} />
            Ask question
          </button>
          <button disabled={busy} className="inline-flex items-center gap-2 rounded-full border border-slate-700/60 bg-zinc-950/50 px-4 py-2 text-sm transition hover:border-slate-400/50 disabled:opacity-50" onClick={() => run({ action: "daily-brief", provider }, "Daily brief generated from current Faust records.")}>
            <FileText size={15} />
            Generate daily brief
          </button>
          <button disabled={busy || !variantId} className="inline-flex items-center gap-2 rounded-full border border-slate-700/60 bg-zinc-950/50 px-4 py-2 text-sm transition hover:border-slate-400/50 disabled:opacity-50" onClick={() => run({ action: "run-scenario", name: "Buy 300 units", prompt: "What happens if I buy 300 units of this SKU?", units: 300, variantId, priceChangePercent: 8, reserveCash: 150 }, "300-unit scenario created.")}>
            <Sparkles size={15} />
            300-unit scenario
          </button>
          <button disabled={busy || !latestRecommendationId} className="rounded-full border border-slate-700/60 bg-zinc-950/50 px-4 py-2 text-sm transition hover:border-slate-400/50 disabled:opacity-50" onClick={() => run({ action: "save-recommendation", recommendationId: latestRecommendationId }, "Top recommendation saved.")}>Save top recommendation</button>
        ...
        <div role="status" className="mt-4 flex items-center gap-2 rounded-2xl border border-slate-700/45 bg-black/35 px-4 py-3 text-sm text-[#c8d2e6]">
          <CheckCircle2 size={15} />
          {status}
        </div>

        {latestAnswer && (
          <div className="mt-4 rounded-2xl border border-slate-700/45 bg-black/35 p-4 text-sm">
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#c8d2e6]">Latest answer</p>
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {prompts.map((prompt) => (
              <button key={prompt} className="rounded-full border border-slate-700/50 bg-zinc-950/40 px-3 py-1.5 text-xs text-muted-foreground transition hover:border-slate-400/60 hover:text-[#edf3ff]" onClick={() => setQuestion(prompt)}>
                {prompt}
              </button>
            ))}
          </div>
