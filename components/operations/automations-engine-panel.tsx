...
export function AutomationsEnginePanel({ data }: { data: OperatingData }) {
  ...
  return <section aria-label="Automation builder" className="faust-surface overflow-hidden">
    <h2 className="border-b border-slate-700/45 px-5 py-4 font-semibold">Rule Builder & Operations Console</h2>
    <div className="grid gap-4 p-5 lg:grid-cols-4">
      ...
      <div className="faust-card p-4"><h3 className="font-semibold">Run</h3><p className="mt-2 text-xs text-muted-foreground">Start a rule, send a Faust event, or process the next background task.</p><div className="mt-4 flex flex-wrap gap-2"><button className={button} disabled={!activeRuleId || busy === "trigger-run"} onClick={() => activeRuleId && run("trigger-run", { ruleId: activeRuleId, samplePayload: { available: 1, reorderPoint: 2, sku: firstActiveVariant?.sku || "FST-HOOD-001" }, idempotencyKey: crypto.randomUUID() })}>Run now</button><button className={button} disabled={busy === "trigger-event"} onClick={() => run("trigger-event", { triggerType: "inventory.below_reorder_point", samplePayload: { id: crypto.randomUUID(), available: 1, reorderPoint: 2, sku: firstActiveVariant?.sku || "FST-HOOD-001", aggregateType: "variant", aggregateId: firstActiveVariant?.id }, idempotencyKey: crypto.randomUUID() })}>Send event</button><button className={button} disabled={busy === "worker-tick"} onClick={() => run("worker-tick", { workerId: "browser-worker", concurrency: 4 })}>Process tasks</button></div></div>
