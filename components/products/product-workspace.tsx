...
export function ProductWorkspace({ item }: { item: ProductExperience }) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/catalog" className="inline-flex items-center gap-2 text-sm font-medium text-red-200 hover:text-red-100"><ArrowLeft size={15} />Back to Products</Link>

      <section className="relative overflow-hidden rounded-[2rem] border border-red-950/45 bg-zinc-950/60 shadow-2xl shadow-black/30 backdrop-blur">
        ...
        <div className="relative grid gap-6 p-5 lg:grid-cols-[340px_1fr] lg:p-7">
          ...
          <div className="flex flex-col justify-between gap-6">
            <div className="flex flex-wrap items-start justify-between gap-5">
              <div>
                <p className="text-xs font-medium uppercase tracking-[0.18em] text-red-300">{item.variant.sku}</p>
            ...
            <div className="rounded-3xl border border-red-950/45 bg-black/35 p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-red-200">Faust recommendation</p>
                  ...
                  <p className="mt-2 text-sm leading-6 text-red-100">{item.intelligence.recommendation.expectedOutcome}</p>
                </div>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <Link href="#marketplaces" className="inline-flex items-center justify-center rounded-full border border-red-950/60 bg-zinc-950/50 px-4 py-2 text-sm font-medium transition hover:border-red-500/50">Review marketplaces</Link>
              </div>
            </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel title="Faust Score">
          <div className="grid gap-5 lg:grid-cols-[170px_1fr]">
            <div className="rounded-[2rem] border border-red-950/45 bg-red-950/15 p-5 text-center">
              <p className="text-sm text-muted-foreground">Overall product health</p>
              <p className="mt-3 font-heading text-6xl font-semibold tabular-nums text-red-50">{item.intelligence.faustScore.score}</p>
              <p className="mt-2 text-sm font-medium text-red-200">{item.intelligence.faustScore.label}</p>
            </div>
            <div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {item.intelligence.faustScore.components.map((component) => (
                  <div key={component.label} className="rounded-2xl border border-red-950/35 bg-black/35 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <b className="text-sm">{component.label}</b>
                      <span className="font-heading text-lg font-semibold tabular-nums text-red-100">{component.score}</span>
                    </div>
        ...
        <Panel title="Product DNA">
          <div className="flex flex-wrap gap-2">
            {item.intelligence.dna.length ? item.intelligence.dna.map((dna) => <span key={dna.tag} className="rounded-full border border-red-900/50 bg-red-950/20 px-3 py-1.5 text-sm text-red-100">{dna.tag}</span>) : <span className="text-sm text-muted-foreground">Faust needs more product history before DNA tags become reliable.</span>}
          </div>
          <div className="mt-4 grid gap-3">
            {item.intelligence.dna.map((dna) => <p key={dna.tag} className="rounded-2xl border border-red-950/35 bg-black/35 p-3 text-sm text-muted-foreground"><span className="font-medium text-foreground">{dna.tag}:</span> {dna.reason}</p>)}
          </div>
        </Panel>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel title="Readiness checklist" id="readiness">
          <div className="grid gap-3 sm:grid-cols-2">
            {item.readiness.dimensions.map((dimension) => (
              <div className="rounded-2xl border border-red-950/35 bg-black/35 p-3" key={dimension.key}>
                <div className="flex items-center gap-2">
                  {dimension.ready ? <CheckCircle2 className="h-4 w-4 text-red-200" /> : <CircleAlert className="h-4 w-4 text-amber-300" />}
        ...
        <Panel title="Inventory and purchasing">
          ...
          <div className="mt-4 rounded-2xl border border-red-950/35 bg-black/35 p-4 text-sm text-muted-foreground">
        ...
        <Panel title="Related products">
          <div className="grid gap-3">
            {item.intelligence.relationships.length ? item.intelligence.relationships.map((relationship) => (
              <Link key={`${relationship.type}-${relationship.href}`} href={relationship.href} className="rounded-2xl border border-red-950/35 bg-black/35 p-3 transition hover:border-red-500/45">
      ...
      <Panel title="Advanced">
        <details className="rounded-2xl border border-red-950/35 bg-black/35 p-4 text-sm text-muted-foreground">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            <Row label="Source URL" value={item.product.sourceUrl ? <Link className="text-red-200" href={item.product.sourceUrl}>Open source</Link> : "Not captured"} />
...
function HeroMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-3xl border border-red-950/35 bg-black/35 p-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-2 font-heading text-2xl font-semibold tabular-nums">{value}</p></div>;
}

function MiniMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-red-950/35 bg-black/35 p-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-2 font-heading text-xl font-semibold tabular-nums">{value}</p></div>;
}
...
function Row({ label, value }: { label: string; value: ReactNode }) {
  return <div className="flex justify-between gap-4 border-b border-red-950/35 py-3 text-sm last:border-0"><span className="text-muted-foreground">{label}</span><span className="text-right">{value}</span></div>;
}

function HealthSignal({ signal }: { signal: ProductExperience["intelligence"]["health"][number] }) {
  const tone = signal.status === "strong" ? "text-red-100" : signal.status === "healthy" ? "text-emerald-200" : signal.status === "watch" ? "text-amber-200" : signal.status === "risk" ? "text-red-200" : "text-muted-foreground";
  return (
    <div className="rounded-2xl border border-red-950/35 bg-black/35 p-4">
...
function MarketplaceRow({ marketplace }: { marketplace: ProductExperience["marketplaces"][number] }) {
  const tone = marketplace.status === "live" ? "text-red-100" : marketplace.status === "rejected" || marketplace.status === "out_of_stock" ? "text-amber-200" : "text-muted-foreground";
  return <div className="flex items-center justify-between gap-3 rounded-2xl border border-red-950/35 bg-black/35 p-3"><MarketplaceBadge marketplace={marketplace.marketplace} /><span className={`text-sm capitalize ${tone}`}>{marketplace.status.replaceAll("_", " ")}</span></div>;
}

function PhotoPlaceholder({ label }: { label: string }) {
  return <div className="grid aspect-square place-items-center rounded-2xl border border-dashed border-red-950/45 text-xs text-muted-foreground">{label}</div>;
}
