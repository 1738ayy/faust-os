...
export function ProductImage({ src, alt, className = "", fallbackClassName = "" }: { src?: string; alt: string; className?: string; fallbackClassName?: string }) {
  ...
  if (!src || failed) return <div className={`grid place-items-center bg-black/35 text-muted-foreground ${fallbackClassName || className}`}><PackageOpen className="h-10 w-10 text-[#edf3ff]/70" /><span className="sr-only">No product image available</span></div>;
