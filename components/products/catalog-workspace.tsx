      ...
      <section className="grid gap-6 md:grid-cols-2 2xl:grid-cols-3" aria-label="Product cards">
        {sortedProducts.map((item) => (
          <div key={item.variant.id} className="space-y-3">
            ...
            <div className="flex flex-wrap justify-end gap-2 rounded-3xl border border-red-950/35 bg-zinc-950/55 p-3">
              <button disabled={busy} className="inline-flex items-center gap-2 rounded-full border border-red-950/60 px-3 py-1.5 text-xs font-semibold text-red-100 transition hover:border-red-500/60 disabled:opacity-50" onClick={() => run("Product duplicated", () => productAction("duplicate", item.variant.id))}><Copy className="h-3.5 w-3.5" />Duplicate</button>
              <button disabled={busy} className="inline-flex items-center gap-2 rounded-full border border-red-950/60 px-3 py-1.5 text-xs font-semibold text-red-100 transition hover:border-red-500/60 disabled:opacity-50" onClick={() => run("Product removed from catalog", () => productAction("delete", item.variant.id))}><Trash2 className="h-3.5 w-3.5" />Delete</button>
            </div>
          </div>
        ))}
      </section>
