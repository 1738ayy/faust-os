...
export function ProductCard({ item }: { item: ProductExperience }) {
  ...
  return (
    <Link
      href={item.href}
      className="group block rounded-[2rem] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-300/25"
      aria-label={`Open ${item.product.title} product workspace`}
    >
      <article className="relative min-h-[560px] overflow-hidden rounded-[2rem] border border-sky-950/45 bg-zinc-950/70 shadow-2xl shadow-black/25 backdrop-blur transition duration-300 motion-safe:group-hover:-translate-y-1 motion-safe:group-hover:shadow-sky-950/20">
        <div className="absolute inset-x-0 top-0 h-48 bg-gradient-to-b from-sky-950/30 to-transparent" />
        <div className="relative h-64 overflow-hidden bg-black/35">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-sky-950/45 bg-black/50 px-2.5 py-1 text-xs text-sky-50">Faust Score {item.intelligence.faustScore.score}</span>
          </div>
        </div>

        <div className="relative p-5">
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-sky-200">{item.variant.sku}</p>
          ...
          <div className="mt-5 rounded-2xl border border-sky-950/35 bg-black/35 p-3">
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-medium text-sky-100">Product health</p>
              <span className="rounded-full bg-sky-950/35 px-2 py-1 text-[11px] text-sky-50">{item.intelligence.faustScore.label}</span>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {item.intelligence.dna.slice(0, 3).map((dna) => <span key={dna.tag} className="rounded-full border border-sky-950/35 bg-sky-950/15 px-2.5 py-1 text-[11px] text-sky-50">{dna.tag}</span>)}
          </div>

          <div className="mt-5 rounded-2xl border border-sky-950/35 bg-black/35 p-3">
            <p className="text-xs font-medium text-sky-100">Faust says</p>
          ...
          <div className="mt-5 flex items-center justify-between gap-3 text-sm">
            ...
            <span className="inline-flex items-center gap-1 font-medium text-sky-100">Open workspace <ArrowRight size={14} /></span>
          </div>
        </div>
      </article>
    </Link>
...
function CardMetric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return <div className="rounded-2xl border border-sky-950/35 bg-black/35 p-3"><p className="text-xs text-muted-foreground">{label}</p><p className={`mt-1 font-heading text-lg font-semibold tabular-nums ${accent ? "text-sky-50" : ""}`}>{value}</p></div>;
}
