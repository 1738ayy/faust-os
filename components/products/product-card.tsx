...
export function ProductCard({ item }: { item: ProductExperience }) {
  ...
  return (
    <Link
      href={item.href}
      className="group block rounded-[2rem] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#8f9bb8]/25"
      aria-label={`Open ${item.product.title} product workspace`}
    >
      <article className="relative min-h-[560px] overflow-hidden rounded-[2rem] border border-slate-700/45 bg-zinc-950/70 shadow-2xl shadow-black/25 backdrop-blur transition duration-300 motion-safe:group-hover:-translate-y-1 motion-safe:group-hover:shadow-slate-950/20">
        ...
        <div className="relative p-5">
          ...
          <div className="mt-5 flex items-center justify-between gap-3 text-sm">
            ...
            <span className="inline-flex items-center gap-1 font-medium text-[#edf3ff]">Open workspace <ArrowRight size={14} /></span>
          </div>
        </div>
      </article>
    </Link>
