...
test("extension side panel exposes safe fill without publishing", () => {
  ...
  assert.match(html, /body\.compact \.expanded-only/);
  ...
  assert.match(script, /data-action="remove"/);
  assert.match(script, /data-action="cover"/);
  assert.match(script, /data-action="crop"/);
  assert.match(script, /extensionViewMode/);
  assert.match(script, /faustSidePanelDraft/);
  assert.match(script, /reorderPhoto/);
  assert.match(script, /saveCrop/);
  assert.match(script, /\/api\/import-image/);
