...
test("migration inventory is ordered and includes the production connection prerequisites", () => {
  ...
  assert.ok(inventory.files.includes("025_product_cover_image_id.sql"));
  assert.ok(inventory.files.includes("026_product_digital_twin_source_revision.sql"));
  ...
  assert.match(guard, /notify pgrst, 'reload schema'/);
  const knowledge = readFileSync("supabase/migrations/029_product_knowledge_engine.sql", "utf8");
  ...
  const visual = readFileSync("supabase/migrations/031_product_visual_intelligence.sql", "utf8");
  ...
  const depop = readFileSync("supabase/migrations/033_depop_production_connector.sql", "utf8");
  assert.match(depop, /marketplace_connector_credentials/);
  ...
  const intelligence = readFileSync("supabase/migrations/035_intelligence_observability_studio.sql", "utf8");
  assert.match(intelligence, /intelligence_decision_timeline/);
  assert.match(intelligence, /intelligence_benchmark_runs/);
  assert.match(intelligence, /intelligence_replay_runs/);
