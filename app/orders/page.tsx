...
export default async function OrdersPage() { const state = snapshot(await getOperatingData()); return <AppLayout><div className="space-y-6"><OrdersDeepeningPanel orders={state.data.orders} variants={state.data.variants} savedViews={state.data.savedOrderViews} notices={state.data.notices} /><OrderImportWorkspace data={state.data} /><OrderDetail data={state.data} /></div></AppLayout>; }
