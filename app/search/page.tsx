            ...
            <div className="space-y-4">
              {groups.map((group) => (
                <section key={group.label} className="faust-surface overflow-hidden">
                  <div className="border-b border-red-950/45 px-5 py-3 text-sm font-medium text-muted-foreground">{group.label}</div>
                  {group.values.map((value) => (
                    <Link key={`${value.href}-${value.label}`} href={value.href} className="block border-b border-red-950/35 px-5 py-4 text-sm transition last:border-0 hover:bg-red-950/15">
                      {value.label}
                    </Link>
                  ))}
                </section>
