      ...
      <div className="space-y-8">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <DataCard title="Incoming parcels" description="Warehouse and international shipments." icon={Plane}>
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-slate-700/35 bg-black/35">{parcels.length === 0 ? <p className="p-6 text-sm text-muted-foreground">No parcels recorded.</p> : parcels.map((parcel) => <div key={parcel.id} className="flex justify-between gap-4 border-b border-slate-700/35 px-5 py-4 last:border-0"><div><p className="font-medium">{parcel.trackingNumber}</p><p className="text-sm text-muted-foreground">{parcel.carrier || "Carrier not set"}</p></div><StatusBadge value={parcel.status} /></div>)}</div>
          </DataCard>
          <DataCard title="Customer orders" description="Sales, fulfillment, and return status." icon={Truck}>
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-slate-700/35 bg-black/35">{orders.length === 0 ? <p className="p-6 text-sm text-muted-foreground">No customer orders recorded.</p> : orders.map((order) => <div key={order.id} className="flex justify-between gap-4 border-b border-slate-700/35 px-5 py-4 last:border-0"><div><p className="font-medium">{order.itemName}</p><p className="text-sm text-muted-foreground">{order.customer} · {order.marketplace}</p></div><div className="text-right"><p className="font-semibold tabular-nums">${order.salePrice.toFixed(2)}</p><div className="mt-1"><StatusBadge value={order.status} /></div></div></div>)}</div>
          </DataCard>
        </div>
