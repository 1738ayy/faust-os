...
export function defaultFinanceCategories(createdAt = now()): FinanceCategory[] {
  ...
  ].map(([name, type]) => ({ id: name.toLowerCase().replaceAll(" ", "_"), name, type: type as FinanceCategory["type"], taxDeductible: ["Software", "Advertising", "Shipping expense", "Packaging", "Freight", "Duty"].includes(name), createdAt }));
}

export function defaultFinancialAccounts(createdAt = now()): FinancialAccount[] {
  return [
    { id: "cash-main", name: "Operating cash", type: "cash", currency: "USD", openingBalance: 0, currentBalance: 0, status: "active", createdAt },
    { id: "marketplace-clearing", name: "Marketplace clearing", type: "marketplace_clearing", currency: "USD", openingBalance: 0, currentBalance: 0, status: "active", createdAt },
    { id: "tax-reserve", name: "Tax reserve", type: "tax_reserve", currency: "USD", openingBalance: 0, currentBalance: 0, status: "active", createdAt },
  ];
}
...
function normalizeExpenses(data: OperatingData, ledger: FinanceLedgerEntry[]): Expense[] {
  ...
  const fromLedger = ledger.filter((entry) => entry.amount < 0 && ["shipping_expense", "packaging", "software", "advertising", "freight", "duty", "expense"].includes(entry.type) && !stored.some((expense) => expense.transactionId === entry.id)).map<Expense>((entry) => ({ id: `expense:${entry.id}`, vendor: entry.description.split(" ").slice(0, 3).join(" "), category: entry.category, amount: Math.abs(entry.amount), date: entry.occurredAt, recurring: entry.type === "software" ? "monthly" : "none", taxDeductible: !["owner_withdrawal", "transfer"].includes(entry.type), receiptStatus: "pending_attachment", orderId: entry.orderId, notes: `Generated from ${entry.sourceLabel}.`, transactionId: entry.id, createdAt: entry.occurredAt }));
...
function normalizePayouts(data: OperatingData, ledger: FinanceLedgerEntry[]): Payout[] {
  ...
  return [...byMarketplace.entries()].map(([marketplace, entries]) => {
    ...
    return { id: `payout:${marketplace.toLowerCase()}:${monthKey()}`, marketplace: marketplace as Payout["marketplace"], status: "expected", expectedAmount: amount, fees: Math.abs(sum(entries.filter((entry) => entry.type.includes("fee")), (entry) => entry.amount)), adjustments: 0, orderIds, transactionIds: entries.map((entry) => entry.id), expectedAt: new Date(Date.now() + 3 * 86400000).toISOString(), createdAt: now() };
  });
}
...
function normalizeBudgets(data: OperatingData, expenses: Expense[]): Budget[] {
  ...
  const generated = categories.filter((category) => !stored.some((budget) => budget.month === currentMonth && budget.category === category)).map<Budget>((category) => {
    ...
    return { id: `budget:${currentMonth}:${category.toLowerCase().replaceAll(" ", "_")}`, month: currentMonth, category, amount, actualAmount, remainingAmount, alertThreshold: 0.85, status: actualAmount > amount ? "overspent" : actualAmount >= amount * 0.85 ? "near_limit" : "on_track", createdAt: now() };
  });
...
function normalizeTaxReserve(data: OperatingData, reconciliations: ReturnType<typeof reconcileOrder>[]): TaxReserveMovement[] {
  ...
  return reconciliations.filter((entry) => entry.contributionProfit > 0).map((entry) => ({ id: `tax:${entry.orderId}`, amount: round(entry.contributionProfit * 0.18), basisAmount: entry.contributionProfit, rate: 0.18, sourceType: "order", sourceId: entry.orderId, status: "reserved", createdAt: now(), notes: `18% reserve from contribution profit on ${entry.number}.` }));
}
...
function normalizeReinvestment(data: OperatingData): ReinvestmentAllocation[] {
  ...
  ].map(([target, percentage]) => ({ id: `allocation:${target}`, target: target as ReinvestmentAllocation["target"], percentage: Number(percentage), amount: 0, basis: "deployable_cash", createdAt: time }));
}
