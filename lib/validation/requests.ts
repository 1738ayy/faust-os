...
export const settingsInputSchema = z.object({ currency: z.string().regex(/^[A-Za-z]{3}$/).optional(), warehouseName: z.string().trim().max(120).optional(), targetMargin: z.coerce.number().min(0).max(100).optional(), defaultMarketplace: z.string().trim().max(50).optional(), depopBoostEnabledByDefault: z.boolean().optional(), depopBoostRate: z.coerce.number().min(0).max(100).optional() });
...
export const listingsActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("save-account-default"), marketplaceAccountId: z.string().uuid(), universalCategoryId: z.string().trim().min(1).max(160).nullable().optional(), fieldKey: z.string().trim().min(1).max(120), value: z.union([z.string(), z.number(), z.boolean(), z.array(z.string())]), enabled: z.boolean().optional(), actor: z.string().trim().max(120).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("save-product-override"), productId: z.string().uuid(), variantId: z.string().uuid().optional(), marketplace: z.enum(["Depop", "eBay", "Etsy", "Mercari", "Poshmark"]), marketplaceAccountId: z.string().uuid().optional(), fieldKey: z.string().trim().min(1).max(120), value: z.union([z.string(), z.number(), z.boolean(), z.array(z.string())]), actor: z.string().trim().max(120).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("publish-product"), productId: z.string().uuid(), marketplaces: z.array(z.enum(["Depop", "eBay", "Etsy", "Mercari", "Poshmark"])).min(1).max(5).optional(), initiatedBy: z.string().trim().max(120).optional(), inventoryStrategy: z.enum(["shared", "allocated"]).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("retry-publish-task"), taskId: z.string().uuid(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("save-draft-field"), draftId: z.string().uuid(), fieldKey: z.string().trim().min(1).max(120), currentValue: z.union([z.string(), z.number(), z.boolean(), z.array(z.string()), z.null()]), actor: z.string().trim().max(120).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("reset-draft-field"), draftId: z.string().uuid(), fieldKey: z.string().trim().min(1).max(120), actor: z.string().trim().max(120).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("save-image-order"), productId: z.string().uuid(), variantId: z.string().uuid().optional(), marketplace: z.enum(["Depop", "eBay", "Etsy", "Mercari", "Poshmark"]), imageIds: z.array(z.string().uuid()).min(1), excludedImageIds: z.array(z.string().uuid()).optional(), coverImageId: z.string().uuid().optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("create-sync-review"), productId: z.string().uuid(), fieldKey: z.string().trim().min(1).max(120), previousValue: z.string().trim().max(500), suggestedValue: z.string().trim().max(500), marketplaces: z.array(z.enum(["Depop", "eBay", "Etsy", "Mercari", "Poshmark"])).min(1).max(5).optional(), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("connect-depop"), displayName: z.string().trim().min(1).max(120).optional(), tokenRef: z.string().trim().min(1).max(200).default("env:DEPOP_API_KEY"), scopes: z.array(z.string().trim().min(1).max(80)).default(["products_read", "products_write", "orders_read", "shop_read"]), mode: z.enum(["api_key", "oauth_pkce"]).default("api_key"), idempotencyKey: z.string().uuid().optional() }),
  z.object({ action: z.literal("disconnect-depop"), idempotencyKey: z.string().uuid().optional() }),
]);
...
export const automationActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("dry-run-event"), triggerType: z.string().min(1), samplePayload: z.record(z.string(), z.unknown()).optional() }),
  z.object({ action: z.literal("set-profile"), profile: z.enum(["manual", "suggested", "assisted", "automatic"]), cooldownMinutes: z.coerce.number().int().min(0).max(1440).optional() }),
