...
export const operatingActionSchema = z.discriminatedUnion("action", [z.object({ action: z.literal("reset") }), z.object({ action: z.literal("transition-order"), id: z.string().uuid(), status: orderStatusSchema }), z.object({ action: z.literal("receive-parcel"), id: z.string().uuid() })]);
