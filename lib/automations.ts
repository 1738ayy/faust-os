...
function variantFromRun(data: OperatingData, run: AutomationRun) { return activeVariants(data).find((entry) => entry.id === run.eventPayload.variantId || entry.sku === run.eventPayload.sku) || activeVariants(data)[0]; }
...
function sampleEventFor(data: OperatingData, type: AutomationTriggerType) { const variant = activeVariants(data)[0], balance = variant ? data.balances.find((entry) => entry.variantId === variant.id) : undefined; return { triggerType: type, sku: variant?.sku || "FST-HOOD-001", available: balance ? availableUnits(balance) : 0, reorderPoint: variant?.reorderPoint || 2, marketplace: data.orders[0]?.marketplace || "Depop", status: "failed", deployableCash: 600, contributionProfit: -1, hoursUntilDeadline: 12, hoursSinceScan: 80, discrepancy: 10, daysWithoutSale: 75, leadTimeDelta: 5 }; }
function valueAt(field: string, payload: Record<string, unknown>, data: OperatingData) { if (field in payload) return payload[field]; if (field === "available") { const variant = activeVariants(data)[0]; const balance = variant ? data.balances.find((entry) => entry.variantId === variant.id) : undefined; return balance ? availableUnits(balance) : 0; } return undefined; }
